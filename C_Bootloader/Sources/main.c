...
void main(void) 
{
  ...
  if(InstallTask(&GPSNET_Apps,"Applications",320,APP3_Priority) != OK)
  {
    while(1){};
  }
  if(InstallTask(&GPSNET_AddressApp,"GPS address task",1280,APP2_Priority) != OK)
  {
    while(1){};
  }   
  
  if(InstallTask(&GPSNET_FOTA_Task,"Bootloader",1280,APP1_Priority) != OK)
  {
    while(1){};
  }  
